...
//adding a param here for the custom module security policy
@description('Optional. Array of security policy objects.')
param securityPolicy array = []
...
module profile_origionGroup 'origingroup/main.bicep' = [for (origingroup, index) in originGroups: {
...
module profile_securityPolicy 'securityPolicy/main.bicep' = [for (securityPolicy, index) in securityPolicy:{
  ...
  params: {
    ...
    profileName: profile.name
    wafPolicy: securityPolicy.wafPolicy
...
@description('CDNServiceManagedIdentityIds of the CDN profile.')
output cdnServiceManagedIdentityIds string = profile.identity.principalId
