...
// batchSize(1) is used to ensure that the subnets are deployed one at a time to avoid concurrency issues with ARM
@sys.batchSize(1)
...
@description('The address prefixes of the deployed subnets.')
output subnetAddressPrefixes array = [for subnet in subnets: subnet.addressPrefix]
