...
module publicIPAddress '../public-ip-address/main.bicep' = if (empty(publicIPResourceID) && isCreateDefaultPublicIP && azureSkuName == 'AZFW_VNet') {
...
module managementIPAddress '../public-ip-address/main.bicep' = if (empty(managementIPResourceID) && isCreateDefaultManagementIP && azureSkuName == 'AZFW_VNet') {
...
@description(' the private IP of the Azure Firewall deployed in virtual hub Subnet.')
output privatehubip string = contains(azureFirewall.properties, 'hubIPAddresses') ? azureFirewall.properties.hubIPAddresses.privateIPAddress : ''
